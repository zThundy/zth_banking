<script lang="ts">
  ...
  async function deleteAllTransactions() {
    if ($transactions.length === 0) {
      ...
      try {
        await fetchNui("fd_banking:client:deleteHistory", {});
        transactions.set([]);
      } catch (error) {
        console.error(error);
      }
  ...
  onMount(async () => {
    try {
      const history = await fetchNui("fd_banking:client:getHistory", {});
...
<div class="relative bg-gray-800 w-full h-full">
  ...
  >
    <div class="bg-gray-700/20 p-8 rounded-lg shadow-lg border border-blue-200/5 h-[calc(100%-.5rem)]">
      <div class="flex items-center mb-6">
        <i class="fa-duotone fa-list text-3xl text-blue-400 mr-3"></i>
        <h2 class="text-3xl font-bold">{$Locales.history}</h2>
      </div>
      ...
      <div class="space-y-4 overflow-auto flex flex-col flex-nowrap align-center jusify-start h-[43rem] align-stretch">
              ...
              <div class="text-right">
                <div class="flex items-center">
                  ...
                  >
                    ...
                    {floorToTwoDecimals(transaction.amount)}
                  </p>
                </div>
              </div>
            </div>
